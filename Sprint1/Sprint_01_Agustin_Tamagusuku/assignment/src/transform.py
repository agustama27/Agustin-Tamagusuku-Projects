...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    filtered_dates = filtered_dates = orders[
    ...
    order_purchase_ammount_per_date = filtered_dates.resample(
        "D", on="order_purchase_timestamp"
        ).order_id.count()
    ...
    result_df = DataFrame()
    result_df["order_count"] = order_purchase_ammount_per_date
